...
import { getEmailTemplate, getSchedulingEmailTemplate } from '../email.js';
...
export const updateAppointmentStatus = async (req, res) => {
  try {
    ...
    const mailOptions = {
      ...
      subject: `Viewing Appointment ${status.charAt(0).toUpperCase() + status.slice(1)} - REALESTATE`,
...
export const scheduleViewing = async (req, res) => {
  try {
    ...
    const mailOptions = {
      from: process.env.EMAIL,
      to: req.user.email,
      subject: "Viewing Scheduled - REALESTATE",
...
export const cancelAppointment = async (req, res) => {
  try {
    ...
    const mailOptions = {
      from: process.env.EMAIL,
      to: appointment.userId.email,
      subject: 'Appointment Cancelled - REALESTATE',
      html: `
...
export const updateAppointmentMeetingLink = async (req, res) => {
  try {
    ...
    const mailOptions = {
      from: process.env.EMAIL,
      to: appointment.userId.email,
      subject: "Meeting Link Updated - REALESTATE",
      html: `
